...
public class AdminControllerTest extends LoginJUnit {
  ...
  public void testGetUser1() throws Exception {
    ...
    String responseStr = getMockMvc()
        .perform(post("/admin/get_user.do").param("user_name", "xiyou").session(getMockHttpSession())).andReturn()
  ...
  public void testModifyUserPwd() throws Exception {
    ...
    String responseStr = getMockMvc().perform(post("/admin/pwdModify.do").param("user_name", "xiyou")
        .param("password", "xiyou").session(getMockHttpSession())).andReturn().getResponse().getContentAsString();
    // System.out.println(responseStr);
    assertEquals("success", JSON.parseObject(responseStr).get("status"));
  }
  ...
  public void testAddBook() throws Exception {
    adminLogin("chengjian", "123456");
    String responseStr = getMockMvc()
        .perform(post("/admin/add_book.do").param("isbn", "9787506391542").param("title", "我喜欢生命本来的样子")
            .param("author", "周国平").param("summary", "测试一下").param("publisher", "作家出版社")
  ...
  public void testDeleteBook() throws Exception {
    ...
    String responseStr = getMockMvc().perform(
        post("/admin/delete_book.do").param("isbn", "9787506391542").param("degree", "1").session(getMockHttpSession()))
        .andReturn().getResponse().getContentAsString();
    // System.out.println(responseStr);
    assertEquals("success", JSON.parseObject(responseStr).get("status"));
  }
  ...
  public void testUpdateBookInfo() throws Exception {
    testAddBook();
    String responseStr = getMockMvc()
        .perform(post("/admin/modify_book.do").param("isbn", "9787506391542").param("degree", "1").param("num", "15")
            .param("actual_price", "16.5").session(getMockHttpSession()))
        .andReturn().getResponse().getContentAsString();
    // System.out.println(responseStr);
    assertEquals("success", JSON.parseObject(responseStr).get("status"));
  }
